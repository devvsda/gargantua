...
int main() {
    ...
    while(T--) {
        ...
        currentStrength = d;
        m.insert(pair<int, int>(currentStrength, 1));
        ...
        while(iter--) {
            ...
            it = m.find(states[currentStrength]);

            if(it == m.end()) m.insert(pair<int, int>(states[currentStrength], 1));
            else it->second++;
