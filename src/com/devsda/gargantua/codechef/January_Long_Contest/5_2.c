            ...
            for(k = 0; k <= i; k++) {
                if( oprtrs[k] == 'A' ) input[ indx[i] ] = ( (input[ indx[i] ]%C) + (A%C) )%C;
                    ...
                    if(C <= 1000000000L ) input[ indx[i] ] = ( (input[ indx[i] ]%C)*(B%C) )%C;
                        ...
                        if(C >= B) input[ indx[i] ] = ( ((C - input[ indx[i] ])) *((C-B)))%C;
                        else input[ indx[i] ] = ( ((C - input[ indx[i] ]))*(B%C) )%C;
                    }
                    else if(input[ indx[i] ] > 1000000000L) {
                        input[ indx[i] ] = (((C - input[ indx[i] ]))*(B) )%C;
                    }
                    else if( B > 1000000000L ){
                        if(C >= B) input[ indx[i] ] = ( (input[ indx[i] ])*((C-B)) )%C;
                        else input[ indx[i] ] = ( (input[ indx[i] ])*(B%C) )%C;
                    }
                    else input[ indx[i] ] = ( (input[ indx[i] ])*(B) )%C;
                }
            }
